...
2. **Existing proxy.** If `install` reports that `ANTHROPIC_BASE_URL` already has another value (for example a caveman proxy on `http://127.0.0.1:8787/w/claude`), write that URL into `~/.cc-hush/config.json` as `upstream` and set `ANTHROPIC_BASE_URL` to `http://127.0.0.1:47831` yourself:
   ```json
   { "upstream": "http://127.0.0.1:8787/w/claude" }
   ```
   Optional `"device": "dml" | "cuda" | "cpu"`; default is `dml` on Windows, `cpu` elsewhere. Restart the daemon after editing: `cc-hush install` or `cc-hush stop` and let the service restart it.
